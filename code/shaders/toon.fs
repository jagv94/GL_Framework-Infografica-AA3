#version 330
	...
	float toonShading(float component){
		if(component >= 0.5) component = 1;
		else if(component >= 0.4) component = 0.4;
		else if(component >= 0.2) component = 0.2;
		else component = 0;
	...
	void main() {
		...
		float diff = max(dot(vert_Normal, normalize(lightDirection)), 0.0);
		
		diff = toonShading(diff);

		vec4 diffuse = (difuseColor * difuseIntensity * diff);
		...
		//specular = step(specular, threshold)
